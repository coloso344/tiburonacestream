...
const VisitorsTab = () => {
  ...
  const fetchVisitors = async () => {
    try {
      console.log("🔍 Cargando visitantes de Supabase...");
      ...
      if (error) {
        ...
        console.log("✅ Visitantes cargados:", data?.length);
  ...
  const { topCountries, topCities, stats } = useMemo(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Mapa simple - SIN LEAFLET */}
      <div className="bg-card border border-border rounded-xl p-6">
        <h3 className="font-display text-sm font-bold text-primary tracking-wider mb-4">
          VISITAS POR PAÍS
        </h3>
        {topCountries.length === 0 ? (
          <p className="text-center text-muted-foreground py-4">Sin datos de países</p>
        ) : (
